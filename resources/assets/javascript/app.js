$(function () {
    ...
    $(".deleteThis").on("click", function (e) {
        ...
        if ($(this).data("confirm-delete") !== "yes") {
            ...
            var that = $(this);
            setTimeout(function () {
                that.data("confirm-delete", "no").html(
                    that.data("original-text")
                );
            }, 2000);
...
function processFormErrors($form, errors) {
    $.each(errors, function (index, error) {
        ...
        if (index.indexOf("[]") > -1) {
            var $formCombinedErrors = $input
                .closest("form")
                .find(".form-errors");
            // $input.addClass('has-error');
            if ($formCombinedErrors.is(":visible") === false) {
                $formCombinedErrors
                    .append('<div class="help-block error">' + error + "</div>")
                    .removeClass("hidden")
                    .addClass("has-error");
            }
            return false;
...
function toggleSubmitDisabled($submitButton) {
    ...
    $submitButton
        .data("original-text", $submitButton.val())
        .attr("disabled", true)
        .addClass("disabled")
        .val("Working...");
}
